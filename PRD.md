...
3 arquivos `.pq` independentes e autocontidos (pt-BR, en-US, es-ES), prontos para copiar e colar no Editor Avançado do Power Query. 100% offline, feriados calculados algoritmicamente, 59 colunas úteis, idênticas em nome e ordem nos 3 arquivos (não a lista completa de 90+ da especificação original).
...
- Sintaxe M: verificada por execução real do parser oficial `@microsoft/powerquery-parser` — passou nos 3 arquivos, inclusive após a correção do bug abaixo.
- Fórmula da Páscoa: verificada manualmente contra 4 datas de referência do prompt original (2000-04-23, 2008-03-23, 2019-04-21, 2026-04-05) — bateram nas 4.
- Fórmula da semana ISO: verificada manualmente contra a virada 2026/2027 (2026-12-31 → 2026-W53; 2027-01-01 → 2026-W53) — bateu.
...
- Bug encontrado e corrigido em revisão posterior: a validação `StartDate > EndDate` nunca era avaliada (M é preguiçoso; nada referenciava a variável de checagem), então o erro explícito exigido pelo escopo nunca disparava. Corrigido nos 3 arquivos referenciando a checagem na construção da lista de datas.
